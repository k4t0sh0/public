  ...
  const firebaseConfig = {
    apiKey: "AIzaSyB-9vVjHgt0sQ9uUvYfA34goLUyl2MkTcM",
    authDomain: "search-word-581d0.firebaseapp.com",
    projectId: "search-word-581d0",
    storageBucket: "search-word-581d0.firebasestorage.app",
    messagingSenderId: "448101823900",
    appId: "1:448101823900:web:b30cca74de67b4ebd2c459",
    measurementId: "G-7JPJ211JY8"
  };
  firebase.initializeApp(firebaseConfig);
const db = firebase.firestore();
const DOC_REF = db.collection("mindmaps").doc("default");
...
async function loadData() {
  try {
    ...
    nodes = JSON.parse(doc.data().nodes);
...
function loadFromPaste() {
    ...
    if (!text) return;
    if (!confirm('現在のデータは上書きされます。続けますか？')) return;
    try {
        const importedNodes = JSON.parse(text);
        if (!Array.isArray(importedNodes) || importedNodes.length === 0)
            throw new Error('配列が空またはフォーマットが無効です');
        const isValid = importedNodes.every(n =>
            n.id !== undefined && n.text !== undefined &&
            n.x !== undefined && n.y !== undefined);
        if (!isValid) throw new Error('不正なデータが含まれています');
        nodes = importedNodes;
        nextId = Math.max(...nodes.map(n => n.id)) + 1;
        selectedNode = null;
        finderSelectedNode = null;
